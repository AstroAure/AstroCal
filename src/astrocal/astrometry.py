...
def plate_solve(hdu, sources, ast=None):
    ...
    wcs_header = ast.solve_from_source_list(sources["xcentroid"], sources["ycentroid"], 
                                            hdu.data.shape[0], hdu.data.shape[0], 
                                            solve_timeout=60, crpix_center=True, parity=0,
...
def astrometry_one(filename, savename, ast=None, mask=False, fwhm=10, threshold=7, verbose=False):
    ...
    wcs = plate_solve(hdu, sources, ast)
...
def astrometry_all(generic_filename,
    ...
    for i, frame in enumerate(list_frames):
        ...
        astrometry_one(frame, f"{savedir}/{save_name}", ast=ast, mask=mask, fwhm=fwhm, threshold=threshold, verbose=verbose)
